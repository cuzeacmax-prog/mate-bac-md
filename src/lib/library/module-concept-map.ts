 ...
 * | VII   | TESTE recapitulative BAC (mixte!)             | ORFAN DELIBERAT — vezi comentariul din hartă |
...
export const MODULE_CONCEPT_ROOTS: Record<string, string[]> = {
  ...
  'Modulul V': MODUL_V,
  'Modulul VI': MODUL_VI,
  // VII = teste recapitulative MIXTE → ORFAN DELIBERAT (listă goală).
  // Dovadă (ETAPA 61, test canonic): cu familia-uniune, integrale din testele VII
  // se linkau la „modulul numărului real" cu sim 0.59–0.62 — poarta de curriculum
  // e vidă la un modul mixt, iar similaritatea singură nu duce greutatea.
  // Mai bine fără link decât link otrăvit. Relinkarea VII cere clasificare
  // PER-EXERCIȚIU (etapă viitoare). Impact produs: zero (VII nu are exerciții verificate).
  'Modulul VII': [],
