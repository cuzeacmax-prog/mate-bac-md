/**
 * ETAPA 50 — Probă: construcția soluției DERIVĂ DIN CONCEPTE (tipare per concept-metodă), nu e inventată
 * per exercițiu. Pentru fiecare exercițiu: concepte → tipare (PATTERNS) → categorii cerute; figura produsă de
 * CAS trebuie să CONȚINĂ acele categorii (poarta STRUCTURALĂ).  npm run verify:construction
 */
...
interface Probe { titlu: string; concepts: string[]; spec: FigureSpec3D | null; reason?: string }
...
// 1) cuboid + diedru — concepte din extractor (registry)
{
  ...
  probes.push({ titlu: "cuboid diedru 45° (AB=8, BC=4)", concepts: r?.concepts ?? [], spec: g.spec, reason: g.reason });
}
// 2) prismă regulată + C₁M — Pitagora în spațiu + înălțime
{
  const g = geo3d({
...
let allOk = true;
console.log("\n════════ CONSTRUCȚIA DERIVATĂ DIN CONCEPTE — poartă STRUCTURALĂ ════════\n");
for (const p of probes) {
  if (!p.spec) { allOk = false; console.log(`✗ ${p.titlu}: figura nu s-a produs (${p.reason ?? "?"})\n`); continue; }
  const { patterns } = requiredFor(p.concepts);
  const chk = checkConstruction(p.spec, p.concepts);
  if (!chk.ok) allOk = false;
  console.log(`${chk.ok ? "✓" : "✗"} ${p.titlu}`);
  console.log(`    concepte folosite : ${p.concepts.join(", ")}`);
...
// CONTROL NEGATIV: cuboid fără marcajul diedrului → poarta structurală PICĂ (cere dihedralMark).
{
  ...
  if (g.spec?.scene) {
    ...
    const chk = checkConstruction(stripped, r!.concepts!);
...
console.log(allOk
  ? "✅ Construcția vine din TIPARELE conceptelor (din teorie), asamblate; poarta cere construcția prezentă."
  : "❌ Construcția derivată din concepte / poarta structurală nu funcționează.");
