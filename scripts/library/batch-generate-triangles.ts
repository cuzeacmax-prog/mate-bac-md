...
async function main() {
  console.log(`\nðŸš€ Batch generator pornit. Limit: ${LIMIT} variaÈ›ii.\n`);

  const supabase = createServiceClient();
  const variations = generateTriangleVariations().slice(0, LIMIT);

  console.log(`ðŸ“‹ ProcesÄƒm ${variations.length} variaÈ›ii.\n`);
  ...
  for (let i = 0; i < variations.length; i++) {
    ...
    console.log(`[${i + 1}/${variations.length}] Procesare variaÈ›ie...`);

    try {
      // Step 1: Generate geometry
      console.log('  âš™ï¸  Calculator geometric...');
      const geo = generateTriangleAdvanced(variation);

      // Step 2: Compile TikZ â†’ SVG
      console.log('  ðŸŽ¨ Compilare TikZ â†’ SVG...');
      const compiled = await compileTikz(geo.tikz);

      // Step 3: Generate statement + solution via Sonnet
      console.log('  ðŸ¤– Generare enunÈ› + soluÈ›ie (Sonnet)...');
      ...
      console.log('  ðŸ§  Generare embedding (Gemini 3072d)...');
